...
if (isset($_GET['delete'])) {
    ...
    $getResident = "SELECT FIRST_NAME, LAST_NAME FROM RESIDENT WHERE RESIDENT_ID = '$id'";
    $residentResult = $con->query($getResident);
    if ($residentResult->num_rows > 0) {
        $resData = $residentResult->fetch_assoc();
        $residentName = $resData['FIRST_NAME'] . ' ' . $resData['LAST_NAME'];
    }
...
// Log action
            if (isset($_SESSION['STAFFID'])) {
                ...
                $description = mysqli_real_escape_string($con, "Deleted resident: $residentName (ID: $id)");
...
// submit modal add start
if (isset($_POST['submitmodal'])) {
    $file_name = $_FILES['profile']['name'];
    $tempname = $_FILES['profile']['tmp_name'];
    $folder = '../admin/Profile/' . $file_name;
    ...
    if ($query_run) {
        ...
        $residentId = mysqli_insert_id($con);
        $residentName = $mfirstname . ' ' . $mlastname;

        // Log action to activity_log
        if (isset($_SESSION['STAFFID'])) {
            $staffId = mysqli_real_escape_string($con, $_SESSION['STAFFID']);
            $action = mysqli_real_escape_string($con, "Add");
            $description = mysqli_real_escape_string($con, "Added new resident: $residentName (ID: $residentId)");
            $targetTable = mysqli_real_escape_string($con, "RESIDENT");
            $targetId = mysqli_real_escape_string($con, $residentId);
            $targetName = mysqli_real_escape_string($con, $residentName);
            ...
            <?php while ($row = $resident->fetch_assoc()): ?>
                <tr>
                    ...
                    <td style="display: flex; justify-content: center; gap: .5rem">
                        <a href="Resident_information.php?delete=<?= $row['RESIDENT_ID'] ?>"><i style="color: gray; font-size: .9rem" class="fas fa-trash"></i></a>
                        <a href="Resident_view.php?Resident=<?= $row['RESIDENT_ID'] ?>"><i style="font-size: .9rem;color:gray" class="fa-solid fa-eye"></i></a>
                    </td>
                </tr>
            <?php endwhile; ?>
        <?php endif; ?>
