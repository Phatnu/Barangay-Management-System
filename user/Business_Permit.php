...
if (isset($_GET['delete'])) {
    ...
    $getBusiness = "SELECT BUSINESS_NAME FROM BUSINESS_PERMIT WHERE ID = '$id'";
    $resultGet = $con->query($getBusiness);
    ...
    if ($result) {
        ...
        if (isset($_SESSION['STAFFID'])) {
            ...
            $description = mysqli_real_escape_string($con, "Deleted business permit: $businessName (ID: $id)");
            $targetTable = mysqli_real_escape_string($con, "BUSINESS_PERMIT");
            $targetId = mysqli_real_escape_string($con, $id);
            $targetName = mysqli_real_escape_string($con, $businessName);
        ...
        echo "<script>alert('Failed to delete business permit!');</script>";
    }
}
...
// ADD NEW BUSINESS PERMIT START
if (isset($_POST['submit'])) {
    // Sanitize inputs to prevent SQL injection
    $permit_num = mysqli_real_escape_string($con, $_POST['PERMIT_NUM']);
    ...
    $business_add = mysqli_real_escape_string($con, $_POST['business_address']);
    $status = mysqli_real_escape_string($con, $_POST['status']);
    $date_app = mysqli_real_escape_string($con, $_POST['date_applied']);
    $until_date = mysqli_real_escape_string($con, $_POST['valid_until']);
    ...
    $business_owner_name = 'Unknown';
    if ($res_query && $res_query->num_rows > 0) {
        $res_data = $res_query->fetch_assoc();
        $business_owner_name = mysqli_real_escape_string($con, $res_data['fullname']);
    }
    ...
    if ($query_run) {
        ...
        if (isset($_SESSION['STAFFID'])) {
            ...
            $description = mysqli_real_escape_string($con, "Added new business permit: $business_name (Permit #: $permit_num)");
            $targetTable = mysqli_real_escape_string($con, "business_permit");
            $targetId = mysqli_real_escape_string($con, $con->insert_id);
            $targetName = mysqli_real_escape_string($con, $business_name);
...
if (isset($_POST['update'])) {
    ...
    $name = mysqli_real_escape_string($con, $_POST['business_name']);
    $type = mysqli_real_escape_string($con, $_POST['business_type']);
    $address = mysqli_real_escape_string($con, $_POST['business_address']);
    $status = mysqli_real_escape_string($con, $_POST['status']);
    $date = mysqli_real_escape_string($con, $_POST['date_applied']);
    $valid = mysqli_real_escape_string($con, $_POST['valid_until']);

    // Get old business name for logging (optional, if you want old and new)
    $oldBusinessName = 'Unknown';
    $getOldName = "SELECT BUSINESS_NAME FROM BUSINESS_PERMIT WHERE ID = '$id'";
    $resultGet = $con->query($getOldName);
    if ($resultGet->num_rows > 0) {
        $data = $resultGet->fetch_assoc();
        $oldBusinessName = $data['BUSINESS_NAME'];
    }
    ...
    if ($con->query($updateSQL)) {
        ...
        if (isset($_SESSION['STAFFID'])) {
            ...
            $description = mysqli_real_escape_string($con, "Updated business permit: $name (ID: $id)");
            $targetTable = mysqli_real_escape_string($con, "BUSINESS_PERMIT");
            $targetId = mysqli_real_escape_string($con, $id);
            $targetName = mysqli_real_escape_string($con, $name);
...
<table class="table table-bordered custom-table table-striped" id="myTable">
  ...
  <tr>
    ...
    <td style="display: flex;justify-content:center;align-items:center;gap:.5rem">
      <a style="color:gray" href="Business_Permit.php?delete=<?= $row['ID'] ?>">
        <i class="fas fa-trash"></i>
      </a>
      <a href="Business_Permit.php?edit=<?= $row['ID'] ?>" style="color:gray">
  <i class="fas fa-pencil-alt"></i>
</a>

      <a style="color:gray" href="permit.php?id=<?=$row['ID'] ?>" target="_blank">
