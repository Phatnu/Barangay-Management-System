...
if (isset($_POST['btnsave'])) {
    ...
    if ($query_run) {
            $_SESSION['toastr'] = "toastr.success('Successfully Added for blotter!');";
...
<table class="table table-bordered custom-table table-striped" id="myTable">
    ...
    <?php while ($row = $resident->fetch_assoc()) { ?>
      ...
      <a style="color:#d1848c" href="Blotter_records.php?delete=<?= $row['ID'] ?>">
        <i class="fas fa-trash"></i>
      </a>
      <a href="Blotter_records.php?edit=<?= $row['ID'] ?>" style="color:#46acc5">
  <i class="fas fa-pencil-alt"></i>
</a>

      <a style="color:#0a9b7e" href="Blotter.php?id=<?=$row['ID'] ?>" target="_blank">
